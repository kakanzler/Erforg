...
- [ ] 本文への画像添付
- [ ] **英字に筆記体（cursive / script）フォントを入れる** — 羊皮紙の見た目に合う
      手書き風の欧文書体。いまは EB Garamond（欧文）＋ しっぽり明朝（和文）。
      当てどころはワードマーク `Erfolg-Forge` と `READING RECORDS`、
      あるいは見出し。本文に使うと可読性が落ちるので避ける。
      注意：**OGP 画像は文字を焼き込んだ JPEG**（`app/opengraph-image.jpg`、Georgia で描画）
      なので、ワードマークの書体を変えるなら描き直しが要る。
- [ ] 「机の上の本 → クリックで開く」オープニング演出

### 検討中：公開サイト上での編集と、その保存先

いまは **ローカルで書く → commit → push** が唯一の経路。手元にPCが無いと書けない。
公開サイトからも書けるようにするなら、更新した `.md` をどこへ保存するかを決める必要がある。

制約は Vercel そのものではなく、**サーバのファイルシステムが読み取り専用**であること。
`content/` に書き戻す現在の API は本番では成立しない。選択肢:

1. **GitHub API でコミットする** — サーバレス関数から GitHub の contents API を叩けば、
   ファイルシステムを介さずに `.md` を commit できる。git 管理・履歴・レビューがそのまま残り、
   ローカル執筆とも共存できる（`/api/records` の書き込み先を dev=fs / 本番=GitHub API に
   切り替えるだけ）。git ベースの CMS が採っている定石でもある。
   要検討：**書き込みに認証が要る**（誰でも書ける状態にはできない）ことと、
   保存のたびに再デプロイが走るので反映に1分ほどかかること。
2. **外部ストレージ**（S3 / R2 / Vercel Blob など）に本文を置き、実行時に読む —
   保存は速いが、`.md` が git から出ていくので履歴も差分レビューも失う。
3. **ヘッドレス CMS** — 重い。この規模には過剰。
4. **現状維持** — ローカル執筆のみ。いちばん単純で、いま困っていないなら妥当。

あわせて、**積読データを git 管理下に置くか**（現在 `content/tsundoku.md` は gitignore で
バックアップ無し）も、リポジトリを private にするかどうかとセットで決める。
